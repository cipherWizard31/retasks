...
import { TASKS, Task, CATEGORY_META } from '../lib/data';

const SUMMARY_CARDS = [
  { label: 'Due Today', value: 4, icon: '📅', color: '#f59e0b', bg: '#fffbeb', border: '#fef3c7' },
  { label: 'Completed', value: 6, icon: '✅', color: '#10b981', bg: '#ecfdf5', border: '#bbf7d0' },
  { label: 'Remaining', value: 4, icon: '⏳', color: '#6b7280', bg: '#f9fafb', border: '#f1f5f9' },
  { label: 'Completion Rate', value: '60%', icon: '📈', color: '#0ea5e9', bg: '#f0f9ff', border: '#e0f2fe' },
];

const UPCOMING_GROUPS = [
  { label: 'Tomorrow', tasks: TASKS.slice(0, 2) }
];
...
export default function DashboardPage() {
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div className="animate-fade-in" style={{
            ...
            <div>
              ...
              <p style={{ margin: '8px 0 0', fontSize: 14, color: '#a7f3d0' }}>
                You have <strong style={{ color: 'white' }}>4 tasks</strong> due today. Let's get them done!
              </p>
            </div>
          ...
          <div className="animate-fade-in delay-100" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            {SUMMARY_CARDS.map((card, i) => (
              <div key={card.label} className="stat-card" style={{ border: `1px solid ${card.border}`, background: card.bg }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{card.label}</div>
          ...
          <section className="animate-fade-in delay-200">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#111827' }}>Today&apos;s Tasks</h2>
                <p style={{ margin: '2px 0 0', fontSize: 13, color: '#9ca3af' }}>{todayTasks.length} tasks scheduled for today</p>
              </div>
            ...
            {todayTasks.length === 0 ? (
              <div className="empty-state">
                ...
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#111827' }}>No tasks due today!</h3>
                <p style={{ margin: '8px 0 0', color: '#9ca3af', fontSize: 14 }}>Enjoy your free time or create a new recurring task.</p>
          ...
          <section className="animate-fade-in delay-300">
            <h2 style={{ margin: '0 0 20px', fontSize: 18, fontWeight: 800, color: '#111827' }}>Upcoming</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {UPCOMING_GROUPS.map((group) => (
                <div key={group.label}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                    <div style={{ height: 1, flex: 0, width: 20, background: '#e5e7eb' }} />
          ...
          <section className="animate-fade-in delay-400">
            <h2 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 800, color: '#111827' }}>Category Overview</h2>
              ...
              {(Object.entries(CATEGORY_META) as [string, typeof CATEGORY_META[keyof typeof CATEGORY_META]][]).map(([key, meta]) => {
                ...
                return (
                  <div key={key} className="card" style={{ padding: '14px 16px', cursor: 'pointer' }}>
                    ...
                    <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 8 }}>{count} tasks</div>
                    <div className="progress-bar">
      ...
      {modalOpen && (
        <CreateTaskModal onClose={() => setModalOpen(false)} />
      )}
    </>
  );
}
