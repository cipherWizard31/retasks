    ...
    <AppShell>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div className="animate-fade-in">
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: '#111827', letterSpacing: '-0.02em' }}>📊 Statistics</h1>
          <p style={{ margin: '4px 0 0', fontSize: 14, color: '#9ca3af' }}>Deep insights into your task performance</p>
        </div>

        {/* KPI cards */}
        <div className="animate-fade-in delay-100" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {[
            { label: 'Avg Completion Rate', value: `${avgRate}%`, icon: '📈', color: '#10b981', bg: '#ecfdf5', sub: 'across all tasks' },
            { label: 'Tasks Completed', value: totalCompleted, icon: '✅', color: '#0ea5e9', bg: '#f0f9ff', sub: 'total all-time' },
            { label: 'Tasks Missed', value: totalMissed, icon: '❌', color: '#ef4444', bg: '#fef2f2', sub: 'total all-time' },
            { label: 'Longest Streak', value: `${longestStreak} days`, icon: '🔥', color: '#f59e0b', bg: '#fffbeb', sub: 'personal best' },
            { label: 'Current Streak', value: '14 days', icon: '⚡', color: '#8b5cf6', bg: '#faf5ff', sub: 'keep it going!' },
            { label: 'Most Consistent', value: bestTask.title, icon: '🏆', color: '#059669', bg: '#ecfdf5', sub: `${bestTask.completionRate}% rate` },
          ].map((kpi, i) => (
            <div key={kpi.label} className={`stat-card delay-${(i + 1) * 100}`} style={{ background: kpi.bg }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{kpi.label}</div>
        ...
        {/* Charts row */}
        <div className="animate-fade-in delay-200" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>

          {/* Weekly Bar Chart */}
          <div className="card" style={{ padding: '24px' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 15, fontWeight: 700, color: '#111827' }}>Weekly Completion</h3>
          ...
          {/* Monthly Line / Category Donut */}
          <div className="card" style={{ padding: '24px' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 15, fontWeight: 700, color: '#111827' }}>Category Breakdown</h3>
            <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
              <div className="donut-ring" style={{ flexShrink: 0 }}>
                <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
                  <div style={{ fontSize: 16, fontWeight: 800, color: '#111827' }}>{totalCatTasks}</div>
                  <div style={{ fontSize: 10, color: '#9ca3af' }}>tasks</div>
                </div>
              </div>
                ...
                {catData.slice(0, 5).map(d => (
                  <div key={d.cat} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#374151', width: 20, textAlign: 'right' }}>{d.count}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="animate-fade-in delay-300 card" style={{ padding: '24px' }}>
          <h3 style={{ margin: '0 0 20px', fontSize: 15, fontWeight: 700, color: '#111827' }}>Monthly Trend</h3>
