...
import { completeTaskAction, uncheckTaskAction, skipTaskAction, editTaskAction, deleteTaskAction } from '../actions/tasks';
...
export default function CalendarClient({ initialTasks }: { initialTasks: Task[] }) {
  ...
  const getTasksForDay = (day: number): Task[] => {
    ...
    return optimisticTasks.filter(t => {
      ...
      if (dateStr === todayStr && (t.status === 'due' || t.status === 'overdue' || t.status === 'completed')) return true;
      if (t.repeatType === 'daily' && t.startDate <= dateStr) return true;
      if (t.repeatType === 'weekly' && t.startDate <= dateStr) {
        const startDayOfWeek = new Date(t.startDate).getDay();
  ...
  return (
    <>
      <AppShell onAddTask={() => setModalOpen(true)}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, opacity: isPending ? 0.85 : 1 }}>
